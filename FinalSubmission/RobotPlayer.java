...
public strictfp class RobotPlayer {
    ...
    static int step = 1;        // Used my most Robots
    static int round = 1;
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        roundBidChecked = Math.max(1, round - 250);  // Check at most 250 rounds before current round
        lock = GameConstants.GAME_DEFAULT_SEED ^ ((rcTeam == Team.A) ? 34645753 : 73453439);   // Blockchain key only works with team-based lock so this can fight against itself
        ...
        switch(rcType) {
            ...
            case MINER:
                seekLoc = null;
                clockwise = ((rcLoc.x + rcLoc.y + round) % 2 == 0) ? true : false;   // Randomize somewhat
        ...
        while (true) {
            ...
            rcLoc = rc.getLocation();
            rcElev = rc.senseElevation(rcLoc);
            waterLevel = GameConstants.getWaterLevel(round);
            ...
            */
            ...
            while(Clock.getBytecodesLeft() > 100 && i < round - 1) {    // Search for highest bid from last checked Blochchain to now, but not all at once
     ...
     * @author Austin Rognes
     * 
     * Miner runs on states and tasks, tasks override states
     * Miner should listen to blockchain commands, find and return soup to refineries, and build all buildings needed
     * 
     */
    static void runMiner() throws GameActionException {
        ...
        if(round > 500 && seekLoc == null && rcState == 0) rcTask = 5;

// Escape rising water TODO make pathfinding avoid water too!
        if(rcElev < waterLevel + 0.1 && rc.isReady()) {
            Direction optimalDir = null;
            Direction waterDir = null;
            boolean scram = false;
            for(Direction checkDir : Direction.allDirections()) {
                if(checkDir == Direction.CENTER) continue;
                ...
                if(rc.canSenseLocation(checkLoc)) {
                    ...
                    if(rc.senseElevation(checkLoc) > rcElev && rc.senseElevation(checkLoc) < rcElev + 4 && rc.canMove(checkDir)) optimalDir = checkDir;
                }
            }
            if(scram) {
                if(optimalDir != null) rc.move(optimalDir);
                else tryMoveLoose(waterDir.opposite());
            }
        }
...
// Conditions to build refinery
        if(cumulativeSoup >= 400 && !refineryNearby && centerX != 0 && centerY != 0 && rcLoc.distanceSquaredTo(hqLoc) > 35 && (refineryLoc == null || rcLoc.distanceSquaredTo(refineryLoc) > 45)) rcTask = 2;
...
// Ready for action
    ...
    // Searching for refinery
                    if(rcState == 2) {
                        ...
                        if(closestRefinery == null) {
                            if(round < 120 || teamSoup < 200) closestRefinery = hqLoc;
    ...
    // Searching for soup
                        ...
                        if(seekLoc != null) pathfind(seekLoc);
                            ...
                            if(rc.canMove(moveDir) && rc.canSenseLocation(rcLoc.add(moveDir)) && !rc.senseFlooding(rcLoc.add(moveDir))) rc.move(moveDir);
...
// Build Design School
                    ...
                    for(Direction checkDir : checkDirs) {
                        MapLocation checkLoc = hqLoc.add(checkDir).add(checkDir).add(checkDir).add(checkDir);
                        if(checkDir.dx != 0 && checkDir.dy != 0) checkLoc = hqLoc.add(checkDir).add(checkDir);
...
// Build Vaporator
                    ...
                    if(!rc.canSenseLocation(hqLoc)/* || rcLoc.distanceSquaredTo(hqLoc) > 25*/) {
                        pathfind(hqLoc);
                        return;
                    }
                    ...
                    if(round < 600) {
// Build in specific spots in base


                        if(teamSoup > 500) {
                            //checkDirs = new Direction[]{centerToHQ, centerToHQ.rotateLeft(), centerToHQ.rotateRight(), hqToCenter.rotateRight().rotateRight(), hqToCenter.rotateLeft().rotateLeft(), hqToCenter.rotateRight(), hqToCenter.rotateLeft(), hqToCenter};
                            for(Direction checkDir : Direction.cardinalDirections()) {
                                MapLocation checkLoc = hqLoc.add(checkDir).add(checkDir).add(checkDir.rotateRight()).add(checkDir.rotateRight());
                                if(rcLoc.distanceSquaredTo(checkLoc) < 3 && tryBuild(RobotType.VAPORATOR, rcLoc.directionTo(checkLoc))) {
                                    rcTask = 3;
                                }
                                else if(rc.canSenseLocation(checkLoc) && !rc.isLocationOccupied(checkLoc)) optimalLoc = checkLoc;
                            }
...
// Build inside base
                        ...
                        if(rcLoc.distanceSquaredTo(hqLoc) < 9) tryMoveLoose(hqLoc.directionTo(rcLoc));
                        ...
                        if(highestDir != null && highestElev - 3 > waterLevel && hqLoc.distanceSquaredTo(rcLoc.add(highestDir)) > 3 && tryBuild(RobotType.NET_GUN, highestDir)) {
                            //rcTask = 3;
                        }

                        if (!tryMoveLoose(clockwise ? hqLoc.directionTo(rcLoc).rotateLeft().rotateLeft() : hqLoc.directionTo(rcLoc).rotateRight().rotateRight())) {
                            clockwise = !clockwise;
                            tryMoveLoose(clockwise ? hqLoc.directionTo(rcLoc).rotateLeft().rotateLeft() : hqLoc.directionTo(rcLoc).rotateRight().rotateRight());
                        }

                    }


                    // Find highest nearby elevation, stick to border BAD IDEA Drones could pick me off



                    // Build vaporators on base outline
                    // Find highest elevation within range
                    /*MapLocation checkLoc = rcLoc;
                    MapLocation highestLoc = null;
                    int highest
                    for(int i = 0; i < 106; i++) {
                        checkLoc = new MapLocation(rcLoc.x + i % 11,  rcLoc.y + (i / 11));
                        if(rc.canSenseLocation(checkLoc) && rc.senseElevation(checkLoc) > highestLoc) highestLoc = checkLoc;
                        rcLoc
                    }*/
                    ...
                    for(Direction checkDir : Direction.allDirections()) {
                        MapLocation checkLoc = rcLoc.add(checkDir);
                        if(rc.canSenseLocation(checkLoc) && rc.senseElevation(checkLoc) > 9) tryBuild( (round % 6 == 0) ? RobotType.NET_GUN : RobotType.VAPORATOR, checkDir);
                    }
                    if(round <= 500) pathfind(hqLoc);
    ...
    static void pathfind(MapLocation endLoc) throws GameActionException {
        ...
        if(boundaryFollow) {    // Follow boundary out of concave shape
            ...
            if(i < 8 && rc.canMove(moveDir) /*&& rc.canSenseLocation(checkLoc) && !rc.senseFlooding(checkLoc)*/) {
                rc.setIndicatorLine(rcLoc, rcLoc.add(moveDir), 255, 0, 0);
                rc.move(moveDir);
            }
...
// Can't move that way
                if(rc.canSenseLocation(checkLoc) ? (Math.abs(rc.senseElevation(checkLoc) - rcElev) > 3 || rc.senseFlooding(checkLoc) || rc.isLocationOccupied(checkLoc)) : false) {
            ...
            if(rc.canMove(optimalDir)) {
                rc.setIndicatorLine(rcLoc, rcLoc.add(optimalDir), 255, 0, 0);
                rc.move(optimalDir);       // Check if can move, just to avoid any possible errors that haven't been accounted for
            }
    ...
    static void runDesignSchool() throws GameActionException {
        ...
        if(rc.getTeamSoup() >= 150 && (minersCreated < 12) && tryBuildLoose(RobotType.LANDSCAPER, rcLoc.directionTo(hqLoc))) {
...
// Sense HQ
        ...
        if(hqLoc == null) System.out.println("Dirt bear hain't found Papa bear");
        if(hqLoc == null || !rc.isReady()) return;   // Don't do anything without hqLoc or if not cooled down
        
// Sense nearby robots, bury enemy buildings
        ...
        for(RobotInfo nrc : nearbyRobots) {
            ...
            if(nrc.getTeam() != rc.getTeam()) {
                ...
                } else if(nrcType == RobotType.LANDSCAPER) {    // Dig up HQ!
                    pathfind(hqLoc);
                    System.out.println("Helping HQ!");
                }
            }
        }
...
// Dig dirt from HQ pits
            ...
            if(rcLoc.distanceSquaredTo(hqLoc) > 2) {
                Direction dirToHq = rcLoc.directionTo(hqLoc);
                if(rc.isLocationOccupied(rcLoc.add(dirToHq)) && rcLoc.distanceSquaredTo(hqLoc) < 9) {

                    if(rc.getDirtCarrying() > 0) {  // If have dirt, find optimal location to place dirt
                        if(rc.canDepositDirt(dirToHq)) rc.depositDirt(dirToHq);
 ...
 // Dig way to battle station
                for(Direction checkDir : checkDirs) {
                    ...
                    if(elevDelta > 3) {    // Dig there
                        if(rc.canDigDirt(checkDir) && rc.isLocationOccupied(rcLoc.add(checkDir))) rc.digDirt(checkDir);
                        else if(rc.canDepositDirt(Direction.CENTER)) rc.depositDirt(Direction.CENTER);
    ...
    // Fortify HQ
                ...
                if(rc.getDirtCarrying() > 0) {  // If have dirt, find optimal location to place dirt
                    ...
                    for(Direction checkDir : Direction.allDirections()) {
                        ...
                        int checkElev = rc.senseElevation(checkLoc);
                        if(checkElev < lowestElev && rc.canDepositDirt(checkDir) && !checkLoc.equals(hqLoc) && ((checkLoc.distanceSquaredTo(hqLoc) != 4 && round < 100) || (checkLoc.distanceSquaredTo(hqLoc) < 3 && round >= 100))) {
                            lowestElev = checkElev;
                            optimalDir = checkDir;
                        }
                    }
                    if(round < 600 && lowestElev > 3) optimalDir = Direction.CENTER;
...
// Terraform around HQ CASE: Low ground
            ...
            if(rc.getDirtCarrying() == 0/* || rcLoc.distanceSquaredTo(hqLoc) > 25*/) rcState = 1;
            if(rcLoc.distanceSquaredTo(hqLoc) > 8 && !tryMoveLoose(rcLoc.directionTo(hqLoc))) pathfind(hqLoc);

// Get dirt from nearest pit
            System.out.println(nearestPit);
            if(rcState == 1 && nearestPit != null) {
                rc.setIndicatorDot(nearestPit, 0, 255, 255);
                
                Direction rcToPit = rcLoc.directionTo(nearestPit);
                if(rcLoc.distanceSquaredTo(nearestPit) > 2) tryMoveLoose(rcLoc.directionTo(nearestPit));//pathfind(nearestPit);
                else if(rc.canDigDirt(rcToPit)) rc.digDirt(rcToPit);
                if(rc.getDirtCarrying() == 25) rcState = 2; // Full of dirt
...
// Dump dirt
                ...
                // Find lowest location (optimal location)
                for(Direction checkDir : Direction.allDirections()) {
                    MapLocation checkLoc = rcLoc.add(checkDir);

                    // Cull pit locations, hqLoc, 
                    int hqDistCheck = hqLoc.distanceSquaredTo(checkLoc);
                    if(hqLoc.equals(checkLoc) || hqDistCheck > 13 || hqDistCheck == 4 || (hqDistCheck == 13 && hqLoc.distanceSquaredTo(checkLoc.add(hqLoc.directionTo(checkLoc).rotateLeft())) == 18) || !rc.canSenseLocation(checkLoc)/* || (rc.isLocationOccupied(checkLoc) && !checkLoc.equals(rcLoc))*/) continue;
                    
                    // Don't destroy buildings until necessary
                    if(rc.isLocationOccupied(checkLoc)) {
                        RobotType rt = rc.senseRobotAtLocation(checkLoc).getType();
                        if((rt == RobotType.DESIGN_SCHOOL || rt == RobotType.REFINERY || rt == RobotType.NET_GUN || rt == RobotType.FULFILLMENT_CENTER || rt == RobotType.VAPORATOR)) continue;
                    } 

                    int checkElev = rc.senseElevation(checkLoc);

                    // Cull extremely low locations
                    if(checkElev < lowestElev && checkElev >= -100) {
                        lowestElev = checkElev;
                        lowestDir = checkDir;
                    }
                }

                // Dump dirt on lowest location if needed, else move around
                if((lowestElev < waterLevel * 16 + 3) && rc.isReady()) rc.depositDirt(lowestDir);
                else {
                    lowestDir = clockwise ? hqLoc.directionTo(rcLoc).rotateRight().rotateRight() : hqLoc.directionTo(rcLoc).rotateLeft().rotateLeft();

                    MapLocation checkLoc = rcLoc.add(lowestDir);
                    if(rc.canMove(lowestDir) && rc.canSenseLocation(checkLoc) && !rc.senseFlooding(checkLoc)) rc.move(lowestDir);
                    else {
                        Direction leftRotate = lowestDir.rotateLeft();
                        lowestDir = lowestDir.rotateRight();

                        if(rc.canMove(leftRotate) && rc.canSenseLocation(rcLoc.add(leftRotate)) && !rc.senseFlooding(rcLoc.add(leftRotate))) rc.move(leftRotate);
                        else if(rc.canMove(lowestDir) && rc.canSenseLocation(rcLoc.add(lowestDir)) && !rc.senseFlooding(rcLoc.add(lowestDir))) rc.move(lowestDir);
                        else {
                ...
                //else tryMoveLoose(rcLoc.directionTo(hqLoc).rotateLeft().rotateLeft()); //if(!tryMoveLoose(clockwise ? rcLoc.directionTo(hqLoc).rotateLeft().rotateLeft() : rcLoc.directionTo(hqLoc).rotateRight().rotateRight())) {
                    //clockwise = !clockwise;
                    //tryMoveLoose(clockwise ? rcLoc.directionTo(hqLoc).rotateLeft().rotateLeft() : rcLoc.directionTo(hqLoc).rotateRight().rotateRight());
                    //System.out.println("Moving!" + rcLoc + " " + hqLoc);
                //}
            }
...
// Dig dirt
            //if(rc.getDirtCarrying() == 0) {
                    for(Direction checkDir : Direction.allDirections()) 
                        if(rc.canSenseLocation(rcLoc.add(checkDir)) && rc.senseElevation(rcLoc.add(checkDir)) > (round / 100 + 5) + 2 && rc.canDigDirt(checkDir)) rc.digDirt(checkDir);
                    for(Direction checkDir : Direction.allDirections()) 
...
// Dump dirt            
            ...
            if(rc.isReady()) {
                ...
                for(int i=8; i >=0; i--) {
                    ...
                    if(((rcLoc.x + hqLoc.x + checkDirs[i].dx) % 2 == 1 ^ (rcLoc.y + hqLoc.y + checkDirs[i].dy) % 2 == 1) && checkElev > -99 && checkElev < (round / 100 + 5) && rc.canDepositDirt(checkDirs[i])) rc.depositDirt(checkDirs[i]);
                }
                //rcState = 2;
            }
            if(rc.isReady()) {
     ...
     * @author Augusto Savaris
     * 
     * Build as many drones as possible
     */
...
// Sense HQ
        ...
        if(rc.getTeamSoup() >= 150 && tryBuildLoose(RobotType.DELIVERY_DRONE, rcLoc.directionTo(hqLoc)) && minersCreated == 0 || round > 600) { // Build one drone for protection early on, then as many as possible
    ...
    /**
     * @author Augusto Savaris & Austin Rognes
     * 
     * Pick up enemies and throw them in water, same for cows.
     * Clump shield HQ late game
     */
...
// Sense HQ
        if(hqLoc == null) { // Find and save hq location if can
            if(rc.sensePollution(rcLoc) == 0) {
                RobotInfo[] nearbyRobots = rc.senseNearbyRobots(rc.getCurrentSensorRadiusSquared(), rcTeam);
                for (int i = 0; i < nearbyRobots.length; i++) {         // HQ location must be within range
                    if (nearbyRobots[i].getType() == RobotType.HQ) {
                        hqLoc = nearbyRobots[i].getLocation();
                        break;
                    }
        ...
        if(hqLoc == null) {
            System.out.println("Airborne bear hain't found Papa bear");
            return;   // Don't do anything without hq location
        }
...
//Shield
        if(round > 800) {
            ...
            // Clump into hq
            if(rcLoc.distanceSquaredTo(hqLoc) > 3) {
                Direction dir = rcLoc.directionTo(hqLoc);
...
// Yeetuth into the drink
        if(rc.isCurrentlyHoldingUnit()) {
            for(Direction dir : Direction.allDirections()) {
                if(rc.senseFlooding(rc.adjacentLocation(dir)) && rc.canDropUnit(dir)) {
                    rc.dropUnit(dir);
                    return;
                }
            }
            tryFlyLoose(hqLoc.directionTo(rcLoc));
        } else {

// Find victims
            MapLocation victimLoc = null;
            int victimId = 0;
            for(RobotInfo ri : rc.senseNearbyRobots()) {
                if(ri.getTeam() != rcTeam) {    // Found potential victim
                    if((victimLoc == null || ri.getLocation().distanceSquaredTo(rcLoc) < victimLoc.distanceSquaredTo(rcLoc))) { // Find closest victim
                        if(ri.getType() == RobotType.COW) { // Initiate animal cruelty
                            victimLoc = ri.getLocation(); 
                            victimId = ri.getID();
                        }
                        else if(ri.getType() == RobotType.MINER || ri.getType() == RobotType.LANDSCAPER) {  // Initiate OSHA violation
                            victimLoc = ri.getLocation(); 
                            victimId = ri.getID();
                        }
                    }
...
// Try snatch victim else follow it
            if(victimLoc != null) {
                Direction rcDirVictim = rcLoc.directionTo(victimLoc);
                if(rc.canPickUpUnit(victimId)) rc.pickUpUnit(victimId);
                else tryFlyLoose(rcDirVictim);

// Circle HQ
            } else {
                if(rcLoc.distanceSquaredTo(hqLoc) < 9) tryFlyLoose(hqLoc.directionTo(rcLoc));
                MapLocation hqClampLoc = new MapLocation(Math.max(Math.min(hqLoc.x, mapWidth - 7), 6), Math.max(Math.min(hqLoc.y, mapHeight - 7), 6));  // Circle the hq, but stay away from corner
                
                if(rc.getLocation().distanceSquaredTo(hqClampLoc) < Math.max(20, round/100 + 16)) tryFlyLoose(rcLoc.directionTo(hqClampLoc).rotateLeft().rotateLeft());
                else tryFlyLoose(rcLoc.directionTo(hqClampLoc));
            }
        }
    }
    ...
    static boolean tryBuildLoose(RobotType type, Direction dir) throws GameActionException {
        ...
        MapLocation checkLoc = rcLoc.add(dir);
        int pitCheck = hqLoc.distanceSquaredTo(rcLoc.add(dir));
        if(rc.canBuildRobot(type, dir) && pitCheck != 4 && !(pitCheck == 13 && hqLoc.distanceSquaredTo(checkLoc.add(hqLoc.directionTo(checkLoc).rotateLeft())) == 18)) {   // Try to build robot at prioritized direction
            rc.buildRobot(type, dir);
            return true;
        }
        ...
        checkLoc = rcLoc.add(leftRotate);
        pitCheck = hqLoc.distanceSquaredTo(rcLoc.add(leftRotate));
        if(rc.canBuildRobot(type, leftRotate) && pitCheck != 4 && !(pitCheck == 13 && hqLoc.distanceSquaredTo(checkLoc.add(hqLoc.directionTo(checkLoc).rotateLeft())) == 18)) {    // Try 1 CC
            rc.buildRobot(type, leftRotate);
            return true;
        }

        checkLoc = rcLoc.add(dir);
        pitCheck = hqLoc.distanceSquaredTo(rcLoc.add(dir));
        if(rc.canBuildRobot(type, dir) && pitCheck != 4 && !(pitCheck == 13 && hqLoc.distanceSquaredTo(checkLoc.add(hqLoc.directionTo(checkLoc).rotateLeft())) == 18)) {           // Try 1 C
            rc.buildRobot(type, dir);
            return true;
        }
        ...
        checkLoc = rcLoc.add(leftRotate);
        pitCheck = hqLoc.distanceSquaredTo(rcLoc.add(leftRotate));
        if(rc.canBuildRobot(type, leftRotate) && pitCheck != 4 && !(pitCheck == 13 && hqLoc.distanceSquaredTo(checkLoc.add(hqLoc.directionTo(checkLoc).rotateLeft())) == 18)) {    // Try 2 CC
            rc.buildRobot(type, leftRotate);
            return true;
        }
        checkLoc = rcLoc.add(dir);
        pitCheck = hqLoc.distanceSquaredTo(rcLoc.add(dir));
        if(rc.canBuildRobot(type, dir) && pitCheck != 4 && !(pitCheck == 13 && hqLoc.distanceSquaredTo(checkLoc.add(hqLoc.directionTo(checkLoc).rotateLeft())) == 18)) {           // Try 2 c
            rc.buildRobot(type, dir);
            return true;
        }
        ...
        checkLoc = rcLoc.add(leftRotate);
        pitCheck = hqLoc.distanceSquaredTo(rcLoc.add(leftRotate));
        if(rc.canBuildRobot(type, leftRotate) && pitCheck != 4 && !(pitCheck == 13 && hqLoc.distanceSquaredTo(checkLoc.add(hqLoc.directionTo(checkLoc).rotateLeft())) == 18)) {    // Try 3 CC
            rc.buildRobot(type, leftRotate);
            return true;
        }
        checkLoc = rcLoc.add(dir);
        pitCheck = hqLoc.distanceSquaredTo(rcLoc.add(dir));
        if(rc.canBuildRobot(type, dir) && pitCheck != 4 && !(pitCheck == 13 && hqLoc.distanceSquaredTo(checkLoc.add(hqLoc.directionTo(checkLoc).rotateLeft())) == 18)) {           // Try 3 c
            rc.buildRobot(type, dir);
            return true;
        }
        ...
        checkLoc = rcLoc.add(dir);
        pitCheck = hqLoc.distanceSquaredTo(rcLoc.add(dir));
        if(rc.canBuildRobot(type, dir) && pitCheck != 4 && !(pitCheck == 13 && hqLoc.distanceSquaredTo(checkLoc.add(hqLoc.directionTo(checkLoc).rotateLeft())) == 18)) {           // Try 4 cc
            rc.buildRobot(type, dir);
            return true;
        }
    ...
    static boolean tryMoveLoose(Direction dir) throws GameActionException {
        ...
        if(rc.canMove(dir) && rc.canSenseLocation(rcLoc.add(dir)) && !rc.senseFlooding(rcLoc.add(dir)) && rc.senseElevation(rcLoc.add(dir)) > round / 100 + 1) {   // Try to move in a prioritized direction
            rc.move(dir);
            return true;
        }
        ...
        if(rc.canMove(leftRotate) && rc.canSenseLocation(rcLoc.add(leftRotate)) && !rc.senseFlooding(rcLoc.add(leftRotate)) && rc.senseElevation(rcLoc.add(leftRotate)) > round / 100 + 1) {    // Try 1 CC
            rc.move(leftRotate);
            return true;
        }

        if(rc.canMove(dir) && rc.canSenseLocation(rcLoc.add(dir)) && !rc.senseFlooding(rcLoc.add(dir)) && rc.senseElevation(rcLoc.add(dir)) > round / 100 + 1) {           // Try 1 C
            rc.move(dir);
            return true;
        }
        ...
        if(rc.canMove(leftRotate) && rc.canSenseLocation(rcLoc.add(leftRotate)) && !rc.senseFlooding(rcLoc.add(leftRotate)) && rc.senseElevation(rcLoc.add(leftRotate)) > round / 100 + 1) {    // Try 2 CC
            rc.move(leftRotate);
            return true;
        }
        if(rc.canMove(dir) && rc.canSenseLocation(rcLoc.add(dir)) && !rc.senseFlooding(rcLoc.add(dir)) && rc.senseElevation(rcLoc.add(dir)) > round / 100 + 1) {           // Try 2 c
            rc.move(dir);
            return true;
        }
     ...
     * @author Austin Rognes
     * 
     * Attempt to fly in a prioritized direction
     * 
     * @param dir    The intended direction of flight
     * @return  true if a flight was performed
     * @throws GameActionException
     */
    static boolean tryFlyLoose(Direction dir) throws GameActionException {
        if(!rc.isReady()) return false;

        if(rc.canMove(dir)) {   // Try to move in a prioritized direction
            rc.move(dir);
            return true;
        }
        ...
        if(rc.canMove(leftRotate)) {    // Try 1 CC
            rc.move(leftRotate);
            return true;
        }

        if(rc.canMove(dir)) {           // Try 1 C
            rc.move(dir);
            return true;
        }
        ...
        if(rc.canMove(leftRotate)) {    // Try 2 CC
            rc.move(leftRotate);
            return true;
        }
        if(rc.canMove(dir)) {           // Try 2 c
            rc.move(dir);
            return true;
        }
    ...
    static void lowPriorityBlockchain(int a, int b, int c, int d, int e) throws GameActionException {
        ...
        for(Transaction t : rc.getBlock(round-2)) 
